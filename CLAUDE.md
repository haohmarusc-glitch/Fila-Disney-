...
Tabelas: `wait_times(ts, park, land, ride, wait_time, is_open, source_updated_at)` — `ts` é quando **nós** coletamos e `source_updated_at` é o `last_updated` que a Queue-Times publicou junto com o número. Os dois divergem, às vezes por horas: em 06/09/2026 o Epic Universe serviu a mesma leitura das 14h46 até depois das 19h, e cada ciclo a regravava com carimbo novo — o histórico passava a afirmar que às 19h15 a fila era 155 min. `leitura_obsoleta` já protegia alerta e ranking; o histórico grava tudo de propósito, e agora grava também a idade, para quem ler depois distinguir leitura nova de repetição. Ausente fica NULL, nunca o `ts` (regra 15 aplicada à idade), e linha anterior à coluna também: não sabemos a idade dela. `migrar_coluna` existe porque `CREATE TABLE IF NOT EXISTS` não alcança banco já criado. Também `alerts_sent(park, ride, sent_at)` `alerts_sent(park, ride, sent_at)` e `daily_summary(sent_on)` — esta última guarda a data (no fuso do parque) em que o resumo das 7h já saiu, para não repetir. `top_alert(id=1, sent_at)` guarda o último envio do alerta de menores filas.

`fila_watches(chat_id, park, ride, limite_min, limite_pct)` é a vigia de fila por pessoa (`/vigiar everest 40`, ou `50%` do típico do horário): no máximo 5 por chat, dispara **uma** vez e se apaga — e o modo percentual só dispara com perfil histórico suficiente, nunca com chute. `atracoes_conhecidas(park, ride, visto_em, avisado)` é o catálogo de tudo que a API já publicou por parque, e serve a duas coisas. Uma é o `/fila <atração>`, o único comando que resolve nome **fora** da watchlist — `/vigiar` e `/confianca` usam `resolver_atracao`, que só olha a watchlist porque precisam de threshold e de perfil. A outra é avisar de atração nova sem repetir: a primeira leitura de um parque entra toda marcada como avisada, senão o primeiro ciclo depois do deploy anunciaria as 76 do Magic Kingdom de uma vez. Depois disso, nome que aparece pela primeira vez rende **um** aviso — o filtro da watchlist continua descartando em silêncio no `/status` e no alerta, que é o certo para a tela, mas agora ele avisa uma vez o que descartou.
